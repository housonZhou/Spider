...
def get_page_url(html_tree, time_map):
    odd_list = html_tree.xpath('//dt[@class="ny_news"]/ul/li')
    data = []
    for item in odd_list:
        try:
            link = item.xpath('a/@href')[0]
            link = 'http://hifda.hainan.gov.cn/zwgk/spaqcj/sjgg' + link[1:]
...
def page_detail(url, link_time):
    ...
    data_list = []
    page_title = page_tree.xpath('string(//div[@class="xx_bt"])').strip()
    need_type = ['xlsx', 'xls', 'csv']
    a_link_list = page_tree.xpath('//p[@class="MsoNormal"]//a')
...
def demo():
    # url = 'http://hifda.hainan.gov.cn/zwgk/spaqcj/sjgg/index.html'
    # data = base_req(url)
    ...
    url = 'http://hifda.hainan.gov.cn/zwgk/spaqcj/sjgg/201901/t20190124_3094166.html'
