...
def get_page_url(html_tree, time_map):
    odd_list = html_tree.xpath('//div[@class="list_list"]/ul[@class="list_one"]/li')
    data = []
    for item in odd_list:
        try:
            link = item.xpath('a/@href')[0]
            link = 'http://www.hljqts.gov.cn/xw/xz' + link[1:]
...
def page_detail(url, link_time):
    ...
    data_list = []
    page_title = page_tree.xpath('string(//div[@class="end_tit"]/h1)').strip()
    need_type = ['xlsx', 'xls', 'csv']
    a_link_list = page_tree.xpath('//div[@class="pt12"]/a')
    ...
    for link_item in a_link_list:
        try:
            ...
            label = re.findall('(.*?)监督', file_name.split('：')[-1])
            if (link_type not in need_type) or ('合格' not in file_name) or not label:
                continue
            label = '-{}-'.format(label[0])
            tag = "_不合格信息_" if "不合格" in file_name else "_合格信息_"
            down_name = "{}{}{}{}@{}.{}".format(page_title, file_name, label, tag, link_time, link_type)
            data_list.append({'url': '{}{}'.format(base_url, link[1:]), 'name': down_name,
...
def demo():
    # url = 'http://www.hljqts.gov.cn/xw/xz/index_1.html'
    # data = base_req(url)
    ...
    url = 'http://www.hljqts.gov.cn/xw/xz/201901/t20190107_119541.html'
