...
def get_page_url(html_tree, time_map):
    odd_list = html_tree.xpath('//tbody/tr')
    data = []
    for item in odd_list:
        try:
            link = item.xpath('td[2]/a/@href')[0]
            link = 'http://syjj.wuhan.gov.cn{}'.format(link)
            link_time = item.xpath('string(td[3])').strip()
...
def page_detail(url, link_time):
    ...
    a_link_list = page_tree.xpath('//a[starts-with(@href,"http://61.183.154.3/directory/DownLoadServlet")]')
    ...
    for link_item in a_link_list:
        try:
            ...
            data_list.append({'url': '{}'.format(link), 'name': down_name,
...
def demo():
    # url = 'http://syjj.wuhan.gov.cn/portal/portalAction!getPage.dhtml?channel_id=595810&forward=info&index=portal_infomationpageWHADpWHDGportal_oachannelWHADs&pagesize=13&clear=true&isSd=false&node_id=GKhubfda&cat_id=595810&pageNo=3&pagenum=3'
    # data = base_req(url)
    ...
    url = 'http://syjj.wuhan.gov.cn/news/595810_482034.html'
