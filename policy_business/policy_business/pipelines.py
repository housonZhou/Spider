...
from policy_business.items import PolicyReformItem
from policy_business.settings import DB_INFO, RUN_LEVEL, PIPELINES_COMMIT
...
class PolicyReformPipeline(object):
    ...
    @ staticmethod
    def insert_sql(keys):
        sql = "INSERT INTO zhengce_business ({}) values ({})".format(', '.join(keys), ', '.join(['%s'] * len(keys)))
        return sql
